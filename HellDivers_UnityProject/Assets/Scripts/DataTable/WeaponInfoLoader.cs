    ...
    private static bool _LoadDataBase(string tablePath, ref Dictionary<int, WeaponInfo> Info)
    {
        ...
        if (datas != null)
        {
            ...
            for (int i = 1; i < lines.Length - 1; i++)
            {
                ...
                data._Center_Damage = int.Parse(weaponInfo[4]);
                data._Explosion_Damage = int.Parse(weaponInfo[5]);
                data._FireRate = float.Parse(weaponInfo[6]);
                data._Capacity = int.Parse(weaponInfo[7]);
                data._Start_Mags = int.Parse(weaponInfo[8]);
                data._Max_Mags = int.Parse(weaponInfo[9]);
                data._Empty_Reload_Speed = float.Parse(weaponInfo[10]);
                data._Tactical_Reload_Speed= float.Parse(weaponInfo[11]);
                data._Min_Spread = float.Parse(weaponInfo[12]);
                data._Max_Spread = float.Parse(weaponInfo[13]);
                data._Spread_Increase_per_shot = float.Parse(weaponInfo[14]);
                data._Range = float.Parse(weaponInfo[15]);
                data._FireMode = float.Parse(weaponInfo[16]);
